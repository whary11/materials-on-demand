...
def generate_laravel5_migration(catalog):
            ...
            for reference in reference_tables:
                for tbl in tables:
                    ...
                    migrations[ti].append(migrationTemplate.format(
                        tableNameCamelCase=("".join(x.title()
                    ...
                    primary_key = [
                        col for col in tbl.indices if col.isPrimary == 1]
                    primary_key = primary_key[0] if len(
                        primary_key) > 0 else None
                        ...
                        try:
                            ...
                            if typesDict[col_type] == 'char':
                                ...
                                if col.precision > -1 and col.scale > -1:
                                    col_data = '\', %s, %s' % (
                                        str(col.precision), str(col.scale))
                            elif typesDict[col_type] == 'double':
                                if col.precision > -1 and col.length > -1:
                                    col_data = '\', %s, %s' % (
                                        str(col.length), str(col.precision))
                            elif typesDict[col_type] == 'enum':
                                col_data = '\', [%s]' % (
                                    col.datatypeExplicitParams[1:-1])
                            ...
                            if col.name == 'remember_token'\
                                ...
                                if col.defaultValue != '' and col.defaultValueIsNull != 0:
                                    ...
                                    if default_value in default_time_values:
                                        migrations[ti].append(
                                            "->default(DB::raw('{}'))".format(default_value))
                                    else:
                                        migrations[ti].append(
                                ...
                                if col.comment != '':
                                    migrations[ti].append(
                                        "->comment('{}')".format(addslashes(col.comment)))
                    ...
                    for index in tbl.indices:
                        ...
                        for column in index.columns:
                            indexes[index_type][index_name].append(
                    ...
                    # Quitando la marca de eliminado para las tablas
                    ...
                    migrations[ti].append(
                        migrationEndingTemplate.format(tableName=table_name))
                    ti += 1
    ...
    for name in sorted(migrations):
        ...
        out.write('Table name: {0}  Migration File: {1}\n\n'.format(
            migration_tables[name], save_format))
...
class GenerateLaravel5MigrationWizardPreviewPage(WizardPage):
    def __init__(self, owner, sql_text):
        ...
        self.save_button.set_tooltip(
            'Select the folder to save your migration(s) to.')
...
class GenerateLaravel5MigrationWizard(WizardForm):
    def __init__(self, sql_text):
        ...
        self.preview_page = GenerateLaravel5MigrationWizardPreviewPage(
            self, sql_text)
